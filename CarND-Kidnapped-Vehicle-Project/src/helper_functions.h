...
inline double  Gaussian(double x_tran, double x_pred, double y_tran, double y_pred, double x_sigma, double y_sigma){
        
   //calculates the probability of x for 1-dim Gaussian with mean mu and var. sigma
  
     return  (1.0/(2.0 * M_PI * x_sigma *y_sigma))*exp(-1.0 * ((pow((x_tran - x_pred), 2)/(2.0 * x_sigma*x_sigma)) + (pow((y_tran - y_pred), 2)/(2.0 * y_sigma*y_sigma))));
